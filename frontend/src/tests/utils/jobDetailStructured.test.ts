...
describe("buildStructuredJobDetail", () => {
  ...
  it("filters Answer/fee junk from representative ISRO job detail payloads", () => {
    const job = {
      detail: {
        source: "pdf_reader_agent",
        summary: "ISRO recruitment notification for Scientist/Engineer posts with application details.",
        fee: {
        ...
        content_sections: [
          {
            heading: "Overview",
            paragraphs: ["Recruitment to Scientist/Engineer posts through valid GATE score."],
            tables: [
              [
                { label: "Answer", value: "There is no written test for the current advertisement. Shortlisting of the" },
                { label: "Technology [Paper Code", value: "CS]" },
    ...
    expect(structured.overviewFacts.some((f) => /paper\s*code/i.test(f.label))).toBe(false);
    expect(structured.overviewFacts.some((f) => /Advt No/i.test(f.label))).toBe(true);
    expect(structured.displaySections.flatMap((s) => s.tables).flat().some((row) => /^answer$/i.test(String(row.label)))).toBe(false);
  });
});
