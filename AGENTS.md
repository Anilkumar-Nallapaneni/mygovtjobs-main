...
npm run db:migrate       # supabase_setup.sql + migrations 001–036
...
- **PDF links:** `utils/resolvePdfUrl.ts` + `utils/officialDomains.ts` — official hosts only (no non-official fallback). Host lists: `shared/official-hosts.json` (FE + BE + audit scripts).
...
2. Run migrations in `database/migrations/` in order (001–036).
3. Backend connects via **Transaction pooler** (`postgresql+asyncpg://…:6543/…`).
4. **CSP:** root `vercel.json` ships `Content-Security-Policy-Report-Only` — do not flip to enforcing until live site shows a clean week of reports.
...
- **Frontend runs fully standalone on committed static data** — `frontend/public/data/live-jobs.json` (homepage catalog is always this file). `VITE_JOBS_SOURCE` only affects job detail / search fallbacks. No Supabase or backend required to browse/search/view job details.
