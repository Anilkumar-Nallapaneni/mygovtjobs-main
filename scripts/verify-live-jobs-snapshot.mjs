...
// Strict publish gates can legitimately leave a very small public catalog after
// deadline/watchdog demotions. Keep the floor hard, but report rolling churn as
// a warning while the snapshot is still in this small-catalog band.
...
export function verifyLiveJobsSnapshot({ strict = false } = {}) {
  ...
  if (previousCount && previousCount >= MIN_PUBLIC_CATALOG_ROWS) {
    ...
    if (fullItems.length < minimumRollingCount) {
      ...
      if (fullItems.length <= SMALL_PUBLIC_CATALOG_WARNING_ROWS) {
        warnings.push(`${message}; allowing because catalog is within the small-catalog warning band`)
      } else {
        issues.push(message)
      }
    }
