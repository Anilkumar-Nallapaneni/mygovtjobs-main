...
// The publish gate can intentionally contract the public catalog to a small set
// of high-confidence live jobs. Keep the hard floor above, but warn instead of
// failing rolling-drop checks for this known small-catalog state.
...
export function verifyLiveJobsSnapshot({ strict = false } = {}) {
  ...
  if (previousCount && previousCount >= MIN_PUBLIC_CATALOG_ROWS) {
    ...
    if (fullItems.length < minimumRollingCount) {
      ...
      if (fullItems.length <= SMALL_PUBLIC_CATALOG_ROWS) {
        warnings.push(`${message}; allowing small gated catalog`)
      } else {
        issues.push(message)
      }
    }
