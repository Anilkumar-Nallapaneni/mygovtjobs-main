...
const siteUrl = process.env.VERCEL_SITE_URL ?? local.VITE_SITE_URL ?? (process.env.GO_LIVE === '1' ? 'https://govtjobs.me' : '')
const googleVerify =
...
if (apiUrl) console.log('  VITE_API_URL =', apiUrl)
  ...
  console.warn('  ⚠ VITE_API_URL not set — admin dashboard needs https://api.govtjobs.me after Railway deploy')
}
